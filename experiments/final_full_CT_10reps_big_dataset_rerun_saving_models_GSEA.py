...
def run(dataset_to_predict: str,
    ...
    configs_to_consider = pd.read_csv("./configs_to_consider_for_GSEA_calc.csv")
    dataset_check = configs_to_consider["config.addl_dataset_train"] == addl_dataset_train
    seed_check = configs_to_consider["config.seed"] == seed
    n_layers_check = configs_to_consider["config.n_layers"] == n_layers
    test_if_run = any(dataset_check & seed_check & n_layers_check)
    ...
    if test_if_run:
        ...
        raise ValueError("Config should not be saved.")
