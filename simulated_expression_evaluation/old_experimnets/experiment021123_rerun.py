...
def run(addl_dataset_train: str,
    ...
    if save_models:
        ...
        # test
        X_loaded = np.load(os.path.join(save_dir,"preds_dbdb_OOD.npy"))
        results_de_dbdb_loaded = compare_de(X=dbdb_ground_truth_OOD.X.toarray(), Y=X_loaded, C=dbdb_ground_truth_h.X.toarray(), shared_top = 1000)
        if results_de_dbdb_loaded["shared_top_genes"] == results_de_dbdb["shared_top_genes"]:
            print("The results are the same, look for bug somewhere else")
        else:
            print("Here is your bug")
